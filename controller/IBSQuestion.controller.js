const IBSQuestion = require("../models/IBSQuestion");
...
const addNewQuestion = async (req, res) => {
  try {
    ...
    const newQuestion = await IBSQuestion.create({
      questionText,
      options,
      isMultipleChoice,
    });
...
const getQuestions = async (req, res) => {
  try {
    const questions = await IBSQuestion.find();
...
const updateNewQuestion = async (req, res) => {
  try {
    ...
    const questionForUpdate = await IBSQuestion.findById(id);
...
const deleteQuestionById = async (req, res) => {
  try {
    await IBSQuestion.findByIdAndDelete(req.params.id);
